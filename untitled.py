...
class Sentiment_Analysis_Model(nn.Module):
    def __init__(self, window_size, vocab_size, device_type):
        ...
        self.xlmr_model.resize_token_embeddings(vocab_size)
        self.dropout = nn.Dropout(p=0.3)
        self.dense = nn.Linear(self.xlmr_model.config.hidden_size, 2)
    ...
    def forward(self, inputs, lengths):
        ...
        (_, pooler_output) = self.xlmr_model(inputs,attention_mask=attention_mask).to_tuple()
